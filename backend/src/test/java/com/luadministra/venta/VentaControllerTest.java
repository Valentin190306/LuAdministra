...
class VentaControllerTest {
    ...
    void listar_retorna200() throws Exception {
        when(service.listar(anyInt(), anyInt(), eq("fecha"), eq("desc")))
                .thenReturn(new PaginatedResponse<>(List.of(makeResponse()), 0, 50, 1, 1));
    ...
    void obtener_retorna200() throws Exception {
        when(service.obtener(1L)).thenReturn(makeResponse());
    ...
    void crear_conDatosInvalidos_retorna400() throws Exception {
        String body = mapper.writeValueAsString(new VentaRequest(null, null));
    ...
    void crear_retorna200() throws Exception {
        when(service.crear(any())).thenReturn(makeResponse());
